...
2. **Resolve the library ID**: Call `resolve-library-id` with:
   - `libraryName`: The library name (e.g., "react", "next.js", "prisma", "heroui")
...
3. **Select the best match**: From the results, pick the library with:
   ...
   - Appropriate version if the user specified one (e.g., "React 19" → look for v19.x, "Next.js 16")
...
- If the question spans multiple distinct concepts, make a separate `query-docs` call per concept with the same library ID
- When the user mentions a version, use version-specific library IDs if available
