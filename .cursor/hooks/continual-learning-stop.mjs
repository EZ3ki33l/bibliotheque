...
async function main() {
  try {
    ...
    const turnsSinceLastRun = state.turnsSinceLastRun + (countedTurn ? 1 : 0);
    const now = Date.now();

    const trialModeOverride = readEnvValue(
      "CONTINUAL_LEARNING_TRIAL_MODE",
      "CONTINUOUS_LEARNING_TRIAL_MODE",
    );
    const trialEnabled =
      trialModeOverride === undefined ? true : parseBoolean(trialModeOverride);
