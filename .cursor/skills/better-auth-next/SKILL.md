...
- Route catch-all : `app/api/auth/[...all]/route.ts`
- Actions : `app/(auth)/actions.ts` — `auth.api.signUpEmail` / `signInEmail` + `headers()`
- Schemas : `app/(auth)/schema.ts` (Zod 4)
- Session RSC : `auth.api.getSession({ headers: await headers() })` dans `app/layout.tsx`

## Patterns

1. Mutations auth = Server Actions, pas `authClient.signIn.email` depuis un form
...
4. Gate admin : table `Admin` (1-1 User), pas un champ `role` sur `User`
5. Ne pas réactiver `cookieCache` sans raison : il est volontairement off

## UI

Toasts via `@heroui/react` (`toast.success`). Pas Sonner.
